...
export default {
  ...
  "setting.remote.setup_table": "\n| 方式 | 详情参考 |\n| --- | --- |\n| 私有服务器自行搭建  |  速度好, 自由配置, 无隐私风险, 访问 <a href='https://github.com/eytb2/fast-note-sync-service'>https://github.com/eytb2/fast-note-sync-service</a> 获取搭建帮助 |",
  ...
  "setting.sync.engine_version": "同步引擎",
  "setting.sync.engine_version_desc": "v3（快照）：git 式清单同步——服务器按整库快照对账，支持重命名追踪与按策略的冲突决策。\nv2（旧版）：逐文件增量同步，保留兼容。\n切换即时生效，两种引擎共用同一份服务器数据。",
  "setting.sync.engine_v2": "v2（旧版逐文件）",
  "setting.sync.engine_v3": "v3（快照清单）",
  "setting.sync.v3_conflict": "v3 冲突策略",
  "setting.sync.v3_conflict_desc": "同一文件双方都修改过时如何裁决（v3 引擎，仅限本端）。\n\n| 策略 | 策略说明 |\n| --- | --- |\n| 最新优先 | 比较本地与服务端修改时间（±2 秒容差），相同取服务端。 |\n| 服务端优先 | 总是用服务端版本覆盖本地。 |\n| 本地优先 | 总是上传本地版本。 |\n| 双方保留 | 保留本地版本，服务端版本另存为冲突副本。 |",
  "setting.sync.v3_strategy_newest": "最新优先",
  "setting.sync.v3_strategy_server": "服务端优先",
  "setting.sync.v3_strategy_local": "本地优先",
  "setting.sync.v3_strategy_copy": "双方保留（冲突副本）",
  ...
  "ui.log.v3.error_repeated": "连续失败 ${count} 次",
  "ui.log.v3.recovered": "已恢复（合并 ${count} 条重复失败）",
